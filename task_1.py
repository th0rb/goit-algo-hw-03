...
def parse_folder(src_path: Path, dest_path: Path):
    try:
...
def copy_file_to_dest(file_path: Path, dest_path: Path):
...
def main():
    parser = argparse.ArgumentParser(description='Copy and sort files on folders based on extensions.')
    ...
    try:
        ...
        print(f"Cannot create Destination folder: {e}")
        sys.exit(1)
